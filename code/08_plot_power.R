...
# set N, range, and number of tested associations (3 EEG variables x 5 Personality traits)
N = 468
lowerlimit = 0
upperlimit = 0.4
Ntests = 15

# Nominal significance: Calculate power of Pearson/Spearman correlation (two.sided)
xy = cbind(NULL, NULL)       
for (i in seq(lowerlimit, upperlimit, length.out = (upperlimit-lowerlimit)*10000+1)){
  calc = pwr.r.test(n = N, r = i, sig.level = 0.05, power = NULL, alternative = "two.sided")
  xy = rbind(xy, cbind(calc$r, calc$power))
}
xy = data.frame(xy)
colnames(xy) = c("r","power")

# Bonferroni-adjusted significance: Calculate power of Pearson/Spearman correlation (two.sided)
xy_bonf = cbind(NULL, NULL)       
for (i in seq(lowerlimit, upperlimit, length.out = (upperlimit-lowerlimit)*10000+1)){
  calc = pwr.r.test(n = N, r = i, sig.level = 0.05/Ntests, power = NULL, alternative = "two.sided")
  xy_bonf = rbind(xy_bonf, cbind(calc$r, calc$power))
}
xy_bonf = data.frame(xy_bonf)
colnames(xy_bonf) = c("r","power")
...
# Open pdf
pdf(paste0(parent,'/figures/power.pdf'), width=10/2.54, height=12/2.54)
par(mgp = c(2, 0.7, 0), lwd=0.5)
plot(xy$r,xy$power,
     type="n",
     main="", 
...
# Insert lines
lines(xy$r, xy$power, col="royalblue3", type="l", lty=2)
lines(xy_bonf$r, xy_bonf$power, col="royalblue3")
...
doty = c(0.2,0.5,0.8)
for (i in 1:3){
dotx = pwr.r.test(n = N, r = NULL, sig.level = 0.05/Ntests, power = doty[i], alternative = "two.sided")$r
text(x = dotx, y = doty[i], labels = bquote(paste("1-", beta," = ", .(doty[i]), ", ", rho, " = ", .(format(round(dotx,digits=3),nsmall=3)))), cex= 0.6, pos=4)
points(x = dotx, y = doty[i], pch=16, col="royalblue3", cex = 0.5)
}
